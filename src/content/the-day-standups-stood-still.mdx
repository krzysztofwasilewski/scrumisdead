...
Teams meet. Attention leaks. We patch the leak with ceremony. **It doesn't hold.**

> Replace status with *signals*. Pair writing replaces tickets for a week. It feels dangerous; that's the point.

```ts
// a meeting that shouldn't have been an email
type Ritual = { name: string; purpose?: string }
const standup: Ritual = { name: "standup" }
```
